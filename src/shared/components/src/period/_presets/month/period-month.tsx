...
import { InputMonth } from '../../../input';
import { validateInputMonth } from '../../../input/_presets/month/input-month.validate';
import { Period } from '../../_core/period';
import type { PeriodMonthProps } from './period-month.types';

export function PeriodMonth(props: PeriodMonthProps) {
  ...
  const endValidator = useCallback((value: string) => {
    const base = validateInputMonth(value);
    if (!base.success) return base;

    const startValue = startValueRef.current;
    if (startValue && value < startValue) {
      return { success: false as const, message: 'Mês/ano final deve ser igual ou posterior ao mês/ano inicial.' };
    }
  ...
  useEffect(() => {
    registerValidator(startName, validateInputMonth);
  ...
  const handleStartBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    formUtils.runValidator(el, validateInputMonth);
